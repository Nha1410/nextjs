    ...
    <section className="relative w-full bg-white bg-cover bg-center">
      <div className="container mx-auto px-6 py-16 md:px-24 lg:px-24">
        <div className="text-left">
          ...
          <h2 className={`text-3xl font-light text-black md:text-5xl lg:text-6xl ${inter.variable}`}>
        ...
        <div ref={topRef.ref} className="mt-8 flex flex-col items-center gap-10 md:flex-row">
          ...
          >
            <Image src="/about1.png" alt="Main Team Image" width={500} height={500} className="w-full max-w-xs rounded-xl md:max-w-sm lg:max-w-md" />
          </div>
        ...
        <div ref={bottomRef.ref} className="mt-16 flex flex-col items-center gap-10 md:flex-row">
          <div className={`order-1 flex-1 space-y-6 px-0 text-left transition-all duration-1000 ease-out ${bottomRef.isInView ? "opacity-100" : "opacity-0"}`}>
            <h3 className={`text-[20px] leading-snug font-bold text-[var(--color-text-red-theme-500)] transition-all duration-700 md:text-[40px] ${bottomRef.isInView ? "translate-y-0 opacity-100" : "-translate-y-5 opacity-0"}`}>
              {content.section2.headline.map((line, idx) => (
                <div key={idx}>{line}</div>
              ))}
            </h3>
            <p className={`text-justify text-base leading-relaxed text-black transition-all delay-300 duration-700 md:text-lg ${bottomRef.isInView ? "translate-y-0 opacity-100" : "-translate-y-5 opacity-0"}`}>
              {content.section2.description}
            </p>
          </div>

          <div className={`order-2 flex flex-1 justify-center transition-all duration-1000 ease-out ${bottomRef.isInView ? "translate-x-0 opacity-100" : "-translate-x-10 opacity-0"}`}>
            <Image src="/about2.png" alt="Thinkers" width={500} height={500} className="w-full max-w-xs rounded-xl md:max-w-sm lg:max-w-md" />
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-6 py-16 md:px-12 lg:px-24">
        ...
        <div className="mt-10 flex flex-col gap-10 md:flex-row">
          ...
          <div ref={valueRef.ref} className="grid flex-1 grid-cols-1 gap-6 text-justify lg:grid-cols-2">
            {content.section3.values.map((val, idx) => (
              ...
              >
                ...
                <h4 className="text-lg leading-snug font-bold text-black md:text-xl">{val.title}</h4>
      ...
      <div className="container mx-auto px-6 py-16 md:px-12 lg:px-24">
        <div className="grid grid-cols-1 gap-8 text-center sm:grid-cols-2 lg:grid-cols-4">
          {content.stats.map((stat, idx) => (
            <div key={idx} className="flex flex-col items-center space-y-2 text-center text-black md:items-start md:text-left">
