export default {
  title1: "Why",
  title2: "Choose Us",
  description:
    "ANN Multimedia đồng hành như một đối tác truyền thông hiểu rõ thị trường, triển khai thực chiến và luôn hướng đến hiệu quả rõ ràng thông qua 4 ưu thế sau:",
  features: [
    {
      number: "01",
      title: "Model",
      description:
        "ANN Multimedia xây dựng theo mô hình toàn diện, được dẫn dắt bởi chiến lược, thúc đẩy bởi sự sáng tạo táo bạo và triển khai thông qua phương tiện truyền thông chính xác. ANN xây dựng sự nhận diện, tăng cường sự tương tác và thúc đẩy sự phát triển trong mọi giai đoạn của hành trình thương hiệu.",
    },
    {
      number: "02",
      title: "People",
      description:
        "Tại ANN, mỗi cá nhân là một sự khác biệt, nhưng tầm nhìn của là chung nhất: biến áp lực thành nguồn cảm hứng và biến ý tưởng thành ảnh hưởng lâu dài. Mỗi tia sáng sáng tạo đều gắn kết với thông tin thị trường, với từng thành viên trong đội ngũ là một nét vẽ đặc trưng trong bức tranh định hình chúng tôi là ai.",
    },
    {
      number: "03",
      title: "Well-Practiced Experiences",
      description:
        "Tại ANN, trải nghiệm được định nghĩa bởi sự triển khai. Mỗi dự án là một chiến lược gắn liền với dữ liệu, được nâng tầm bởi sự sáng tạo, và được đo lường qua kết quả thực tế. Thành tích không chỉ được ghi lại trong các báo cáo mà còn thể hiện qua sự phát triển bền vững của các thương hiệu đã từng hợp tác.",
    },
    {
      number: "04",
      title: "In-Depth Insight in The Local Market",
      description:
        "Những hiểu biết sâu sắc của ANN được xây dựng trên sự tham gia trực tiếp, quan sát và trải nghiệm thị trường cùng với người dùng thực tế. Sự gần gũi này giúp các thương hiệu giao tiếp một cách có liên quan và chân thực, không phải như những kẻ ngoài cuộc, mà là những tiếng nói đáng tin cậy trong cộng đồng.",
    },
  ],
};
